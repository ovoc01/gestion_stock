...
export default function ReadOnlyTable({ headers, data, title }: ReadOnlyTableProps) {
            ...
            <tr className="text-gray-500 border-b border-gray-500">
               {
                  headers.map(({ key, label }) => (
                     <th key={key} className="font-light text-start">{label}</th>
                  ))
               }
            </tr>
         </thead>
         <tbody className="">
            {
               data?.map((record, index) => (
                  <tr key={index} className="text-gray-500 border-b border-gray-300">
                     {
                        headers.map(({ key }) => (
                           <td className="font-light text-start py-4 ">
                              {record[key]}
                           </td>
                        ))
                     }
