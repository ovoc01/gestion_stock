...
import { exportValorisationStock, exportValorisationStockPDF, getValorisationStock } from "@/services/api/stock.service"
import { EmplacementDataProps, MagasinDataProps, ValorisationDetails } from "@/types/types"
...
export default function ValorisationStock() {
   ...
   const initialEmplId = searchParams.get('emplId') ? parseInt(searchParams.get('emplId')!) : null;
   const initialMagId = searchParams.get('magId') ? parseInt(searchParams.get('magId')!) : null;
   const initialCodeArticle = searchParams.get('codeArticle') ? searchParams.get('codeArticle') : null;
   ...
   const [valorisationDetails, setValorisationDetails] = useState<ValorisationDetails[] | null>(null)
   ...
   const handleFilterChange = (newEmplId?: number, newMagId?: number, newCodeArticle?: string) => {
      ...
      const updateQueryParam = (key: string, newValue?: any, initialValue?: any) => {
   ...
   const exportExcel = async () => {
      const response = await exportValorisationStock(initialMagId!, initialEmplId!, initialCodeArticle!)
      await downloadFile(response, DownloadType.EXCEL)
   }

   const exportPDF = async () => {
      const response = await exportValorisationStockPDF(initialMagId!, initialEmplId!, initialCodeArticle!)
   ...
   return (
      <div className="w-full flex h-fit  flex-col item-center justify-between gap-11 h-full">
         <h1 className="text-6xl font-bold text-primary text-center">Valorisations de Stock</h1>
         <div className="w-full flex h-full justify-around">
            <div className="w-4/6">
               {
                  valorisationDetails ? (<>
                     <ReadOnlyTable headers={headers} data={valorisationDetails!} />
                     <div className="flex flex-col gap-2">
                        <h1 className="text-3xl font-medium mt-5">Valorisations: <span className="text-primary font-semibold">{formatCurrency(valorisationStock!)}</span></h1>
                     </div>
                     <div className="flex w-1/5 gap-5 mt-4">
                        <Button className=" bg-lime-400 text-light rounded-md" onPress={exportExcel}>
                           EXCEL
                        </Button>
                        <Button className="bg-primary text-light rounded-md" onPress={exportPDF}>
                           PDF
                        </Button>
                     </div>
                  </>
            ...
            <div className="w-2/6 max-w-[300px] max-h-[350px] flex  flex-col gap-3 pb-8 border-solid border-1  border-gray-300 rounded-lg shadow-md p-8  mt-8 sticky">
               <h1 className="text-3xl font-thin flex items-center justify-center w-full gap-8">Filtre
               ...
               <div className="flex w-full gap-4">
                  <Select
                     ...
                     onChange={(e) => {
                        handleFilterChange(undefined, parseInt(e.target.value), undefined)
                     }}
                  >
                     {magasins!.map((mag) => (
               ...
               <div className="flex w-full gap-4">
                  <Select
                     ...
                     onChange={(e) => {
                        handleFilterChange(parseInt(e.target.value), undefined, undefined)
                     }}
                  >
