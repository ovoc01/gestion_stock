      ...
      <div className="w-3/5 h-full pb-8 border-solid border-1  border-gray-300 rounded-lg shadow-md p-8">
        <div className="about flex flex-col gap-4 ">
          <div className="flex justify-between items-center">
            ...
            <Button className=" rounded-md bg-primary text-white" size="md">
              Ajouter Emplacement
            </Button>
          ...
          <h1 className="text-md text-primary">
            ...
            <span className="font-bold text-black ml-2">
              {magasin?.info.utilisateurs}
            </span>
          </h1>
          <h1 className="text-md text-primary">
            Nombre d'Emplacement:
            <span className="font-bold text-black ml-2">
              {magasin?.info.nombreEmplacement}
            </span>
          </h1>
        </div>
          ...
          <table className=" mt-4 table-auto ">
            ...
            <tbody className="">
              {magasin?.stocks?.map((stock) => (
                <tr
                  key={stock.code_article}
                  className="text-gray-500 border-b border-gray-300"
                >
                  <td className="font-light text-center py-4">
                    {stock.emplacement}
                  </td>
                  ...
                  <td className=" text-center text-primary italic">
                    {formatCurrency(stock.cmup)}
                  </td>
                  <td className="font-bold text-lg text-center text-primary">
                    {formatCurrency(stock.prixTotal)}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className=" mt-5 flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2">
            <h1 className="text-lg ">
              Montant Total Sortie:{" "}
              <span className="text-primary font-semibold">
                20 000 000,00 Ar
              </span>
            </h1>
            <h1 className="text-lg ">
              Montant Total Entrée:{" "}
              <span className="text-primary font-semibold">
                20 000 000,00 Ar
              </span>
            </h1>
          ...
          <h6 className="text-sm italic ">
            Nb: * Les montants affichés sont des montants estimatifs et peuvent
            varier en fonction des mouvements de stock et ils sont tirés du
            periodes actuels.
          </h6>
        </div>
      </div>
      <Map center={unopPosition!} className=" w-2/5 h-full" markers={markers} />
    </div>
  );
}
