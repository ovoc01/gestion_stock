...
export default function DashboardPage() {
   ...
   useEffect(() => {
      getUtilisateursActiveNumber().then((data) => {
      ...
      return <div className="w-full flex  h-[100%]  gap-8 py-8">
         <div className=" p-8 border-solid flex flex-col gap-6 h-full w-4/6">
            <div className="w-full flex gap-5 ">
               <StatsCard number={userCount} title="Utilisateur Active" icon={faUsers} percentage="" />
            </div>
   ...
   const renderPage = isLoading ? <CSpinner label="Récuperation de donnée" /> : <Index />
   return renderPage;
}