...
public class SousFamilleService {
    ...
    public List<SousFamilleDTO> getAllEntities(int page, int size, Fetch fetch) {
        if (fetch == Fetch.ALL)
            return sousFamilleRepository.findAllSousFamilles();
    ...
    public void saveEntity(SousFamilleRequest request) {
        ...
        sousFamilleRepository.createSousFamille(request.getSousFamLi(), request.getFamilleId(),
                famille.getFamLogRef() + createFamilleLogRef(request.getSousFamLi()));
    }
